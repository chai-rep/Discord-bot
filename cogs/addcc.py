...
# ---------- Import your DynamoDB table directly ----------
from config import CLASSES_TABLE  # only the classes table

class AddCC(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.table = CLASSES_TABLE  # use the imported table

    # ---------- Helper Functions ----------
    def class_exists_by_role(self, role_id: str):
        try:
            response = self.table.scan(
                FilterExpression=Attr('roleID').eq(role_id)
            )
    ...
    def add_class_to_dynamodb(
        self, class_code, channel_ids, role_id, title,
    ...
    async def addcc(
        ...
        if existing_role_class and existing_role_class['classCode'] != class_code:
            await interaction.followup.send(
                f"❌ This role already has class code {existing_role_class['classCode']}."
            )
            return
        ...
        # ---------- Validate channels ----------
        all_channel_ids = [cid.strip() for cid in channel_ids.split(',')]
        valid_channels = []
        for cid in all_channel_ids:
            try:
                channel = interaction.guild.get_channel(int(cid))
            except ValueError:
                await interaction.followup.send(f"❌ Invalid channel ID: {cid}")
                return
            if not channel:
                await interaction.followup.send(f"❌ Channel ID {cid} not found in server.")
                return
        ...
        if success:
            await interaction.followup.send(
                f"✅ Class `{class_code}` added successfully!\n"
                f"Title: {title}\nRole: {role.mention}\n"
                f"Channels: {', '.join(valid_channels)}"
            )
        else:
